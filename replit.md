...
-   **Key Features**:
    ...
    -   **Transactions**: Handles net weight calculation, commission application (aadhat, mandi, muddat+anya), and management of payable/receivable amounts. It supports customizable business charge settings with versioned history (each save creates a new row; dashboard shows change history). Existing transactions preserve the charge percentages they were created with, preventing retroactive recalculation when global settings change.
    -   **Cash Management**: Tracks cash inflows and outflows, categorizing expenses (Revenue/Capital) and supporting payment allocations. Payment creation uses database transactions with row-level locking (`SELECT ... FOR UPDATE`) to prevent concurrent duplicate/overpayments against the same transaction. Supports **Buyer Advance Payment** (excess amount deposited as advance via allocation picker "Advance" option, stored in `advanceAmount` column) and **Advance Adjustment** (4th payment mode "Advance Adj" that uses buyer's advance balance to settle outstanding dues without affecting cash/bank balances). Advance balance = sum(advanceAmount from non-reversed entries) minus sum(amounts from Advance Adj entries). Reversal guard prevents reversing advance deposits when advance has been consumed by Advance Adj entries.
