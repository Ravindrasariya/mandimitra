...
- Farmer fields: farmerId (auto-generated FM+YYYYMMDD+seq, unique per business), name, phone, village, tehsil, district, state, openingBalance, negativeFlag (business-level red flag warning — indicates caution, not related to credit/debit), isArchived
...
- Buyer fields: buyerId (auto-generated BY+YYYYMMDD+seq), name, phone, address, buyerCode, negativeFlag (business-level red flag warning — indicates caution, not related to credit/debit), isActive, openingBalance
- Lot fields: lotId (auto POT/ONI/GAR+YYYYMMDD+seq), crop, variety, numberOfBags (original, fixed at entry), actualNumberOfBags (editable, defaults to numberOfBags, can be reduced for damaged/graded harvest), remainingBags (= actualNumberOfBags - sold), size, bagMarka, vehicleNumber, vehicleBhadaRate, initialTotalWeight
...
- Transaction fields: transactionId (auto-generated TX+YYYYMMDD+seq, unique per business+date, not displayed in UI), paidAmount (decimal, tracks FIFO-allocated payment), paymentStatus (due/partial/paid, auto-calculated on buyer cash entry create/reverse), includes split charge fields (aadhatFarmerPercent, aadhatBuyerPercent, mandiFarmerPercent, mandiBuyerPercent, hammaliFarmerPerBag, hammaliBuyerPerBag) auto-populated from business charge settings. Extra charges: extraChargesFarmer and extraChargesBuyer (editable per-transaction, not from settings). Stored totals: hammaliCharges, extraChargesFarmer, extraChargesBuyer, freightCharges, aadhatCharges, mandiCharges. Freight/Bhada: auto-calculated from lot's vehicleBhadaRate × proportionate original bags ((bidBags × originalBags / actualBags) × rate), deducted from farmer.
